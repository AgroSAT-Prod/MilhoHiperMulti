...
# Desabilitar logs excessivos do Optuna
...
def otimizar_rf(X_train, y_train, n_trials=20, scoring='f1_weighted'):
    """
    Executa o Optuna para encontrar os melhores hiperparâmetros para o Random Forest.
    """
    print(f"   ⚙️ Iniciando otimização com Optuna ({n_trials} trials)...")
    ...
    def objective(trial):
        ...
        # Pipeline temporário para validação cruzada
        pipeline_opt = Pipeline([
            ('smote', SMOTE(random_state=SEED, k_neighbors=1)),
            ('rf', RandomForestClassifier(**param_grid, random_state=SEED, n_jobs=-1, class_weight='balanced'))
        ])
        
        # Cross-validation para evitar overfitting nos dados de teste final
        cv = StratifiedKFold(n_splits=3, shuffle=True, random_state=SEED)
        
        try:
            scores = cross_val_score(pipeline_opt, X_train, y_train, cv=cv, scoring=scoring, n_jobs=-1)
            return scores.mean()
        except Exception as e:
            return 0.0 # Retorna 0 em caso de erro para o Optuna ignorar essa tentativa

    study = optuna.create_study(direction='maximize')
    study.optimize(objective, n_trials=n_trials)
    
    print(f"   ✓ Melhores parâmetros encontrados: {study.best_params}")
    print(f"   ✓ Melhor Score ({scoring}): {study.best_value:.4f}")
...
def classificar_multiclasse(X, y, cols_nomes, titulo="Classificação Multi-Classe (Doses)"):
    ...
    # Otimizamos APENAS nos dados de treino para evitar vazamento de dados (data leakage)
    best_params = otimizar_rf(X_train, y_train, n_trials=N_TRIALS, scoring='f1_weighted')
    ...
    # plt.show() # Comentado para execução em fluxo contínuo
    plt.close()
...
def classificar_binaria(X, y_dose, cols_nomes, titulo="Classificação Binária (Com/Sem N)"):
    ...
    # --- OTIMIZAÇÃO COM OPTUNA ---
    # Para binária, usamos ROC_AUC como métrica de otimização, mas F1 também serve
    best_params = otimizar_rf(X_train, y_train, n_trials=N_TRIALS, scoring='f1')
    
    # Pipeline com SMOTE + Random Forest Otimizado
    pipeline = Pipeline([
        ('smote', SMOTE(random_state=SEED, k_neighbors=1)),
        ('rf', RandomForestClassifier(**best_params, class_weight='balanced', random_state=SEED, n_jobs=-1))
    ])
...
def plotar_comparacao_metricas(resultados_multi, resultados_bin):
    ...
    plt.suptitle('Comparação de Métricas: Multi-Classe vs Binária (Otimizado)', fontsize=16, fontweight='bold')
    plt.tight_layout()
...
if __name__ == "__main__":
    print("="*70)
    print(" CLASSIFICAÇÃO - RESPOSTA AO NITROGÊNIO (COM OPTUNA)")
    print("="*70)
