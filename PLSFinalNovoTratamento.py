...
def cruzar_com_excel(df_espectral, arquivo_excel):
    ...
    # --- ALTERAÇÃO AQUI: Prioriza 'tratamento', depois tenta 'dose' ---
    ...
    # Cria target de clorofila (opcional, mantido do original)
    ...
    # Renomeia a coluna alvo para 'Tratamento' independente se veio de 'dose' ou 'tratamento'
    if col_alvo:
        df_merge.rename(columns={col_alvo: 'Tratamento'}, inplace=True)
    else:
        df_merge['Tratamento'] = 0 # Valor default se não achar nada
    
    cols_finais = list(df_espectral.columns) + ['Tratamento', 'Y_Clorofila']
    # Garante que as colunas existem antes de retornar
...
def classificar_binaria(X, y_numerico, cols_nomes, titulo="Classificação Binária (Com/Sem N)"):
    """
    Classificação binária: Com Nitrogênio vs Sem Nitrogênio.
    Baseia-se no valor numérico do tratamento (se > 0 considera Com N).
    """
    
    # Criar labels binários
    # Se o valor for 0 (zero), é Sem N. Qualquer outro valor é Com N.
    ...
    try:
        ...
        roc_auc = 0.5 # Fallback caso só tenha 1 classe no teste
    ...
    if len(np.unique(y_test)) > 1:
        ...
        ax2.text(0.5, 0.5, "Apenas uma classe no teste", ha='center')
...
if __name__ == "__main__":
    ...
    # Prepara variavel alvo Tratamento (String/Categorica para Multi)
    y_tratamento = df['Tratamento'].astype(str)
    
    # Prepara variável alvo Numérica (para Binária: > 0 é tratado)
    # Se 'Tratamento' for texto puro (ex: "T1"), isso pode falhar. 
    # Tenta converter para numero. Se falhar, assume 0.
