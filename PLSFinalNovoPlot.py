...
from sklearn.tree import plot_tree  # <--- IMPORT ADICIONADO
...
def classificar_multiclasse(X, y, cols_nomes, titulo="Classificação Multi-Classe (Doses)"):
    """Classificação multi-classe para as 4 doses com plotagem de árvore."""
    ...
    # ==========================================================================
    # VISUALIZAÇÃO DA ÁRVORE DE DECISÃO (NOVO CÓDIGO)
    # ==========================================================================
    print(f"\n  Gerando visualização da Árvore de Decisão...")
    
    # Extrair uma árvore individual da floresta (índice 0)
    arvore_individual = rf_model.estimators_[0]
    ...
    plot_tree(arvore_individual, 
              feature_names=cols_nomes,            # Nomes das bandas (ex: Band_430)
              class_names=le.classes_.astype(str), # Nomes das classes (0, 90, 180, 360)
              filled=True,                         # Cores preenchidas (estilo da imagem)
              rounded=True,                        # Bordas arredondadas
              fontsize=10,                         # Tamanho da fonte
              max_depth=3)                         # Limita a profundidade para não poluir
    
    plt.title(f"Visualização de uma Árvore do Random Forest - {titulo}", fontsize=16, fontweight='bold')
    plt.tight_layout()
    
    nome_arquivo_tree = f'arvore_decisao_{titulo.lower().replace(" ", "_")}.png'
    plt.savefig(nome_arquivo_tree, dpi=300)
    print(f"  ✓ Salvo: {nome_arquivo_tree}")
    plt.show()
    # ==========================================================================
