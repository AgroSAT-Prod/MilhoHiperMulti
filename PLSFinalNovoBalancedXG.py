...
def cruzar_com_excel(df_espectral, arquivo_excel):
    """Cruza dados espectrais com agronômicos do Excel (Versão Corrigida)."""
    try:
    ...
    # Remove Dose_N duplicada do CSV original antes do merge
...
# ================= CLASSIFICAÇÃO MULTI-CLASSE (XGBOOST) =================

def classificar_multiclasse(X, y, cols_nomes, titulo="Classificação Multi-Classe (Doses)"):
    """Classificação multi-classe usando XGBoost."""
    
    # Codificar labels (Obrigatório para XGBoost: deve ser 0, 1, 2...)
    le = LabelEncoder()
    ...
    pipeline = Pipeline([
        ...
        ('xgb', XGBClassifier(
            n_estimators=200,          # Mais árvores que o padrão
    ...
    ax.set_title(f"Matriz de Confusão - {titulo} (XGBoost)", fontsize=14, fontweight='bold')
    ax.set_xlabel("Predito (kg/ha)", fontsize=12)
    ax.set_ylabel("Verdadeiro (kg/ha)", fontsize=12)
    plt.tight_layout()
    plt.savefig(f'matriz_confusao_xgb_{titulo.lower().replace(" ", "_")}.png', dpi=300)
    print(f"  ✓ Salvo: matriz_confusao_xgb_{titulo.lower().replace(' ', '_')}.png")
    plt.show()
    ...
    plt.tight_layout()
    plt.savefig(f'importancia_xgb_multi.png', dpi=300)
    print(f"  ✓ Salvo: importancia_xgb_multi.png")
    plt.show()
...
def classificar_binaria(X, y_dose, cols_nomes, titulo="Classificação Binária (Com/Sem N)"):
    ...
    pipeline.fit(X_train, y_train)
    y_pred = pipeline.predict(X_test)
    y_pred_proba = pipeline.named_steps['xgb'].predict_proba(X_test)
    ...
    ax2.set_title('Curva ROC (XGBoost)', fontsize=12, fontweight='bold')
    ax2.legend(loc="lower right", fontsize=10)
    ax2.grid(True, alpha=0.3)
    
    plt.tight_layout()
    plt.savefig(f'matriz_roc_xgb_{titulo.lower().replace(" ", "_")}.png', dpi=300)
    print(f"  ✓ Salvo: matriz_roc_xgb_{titulo.lower().replace(' ', '_')}.png")
    plt.show()
    
    # Importância das Features (Binária)
    model = pipeline.named_steps['xgb']
    importancias = model.feature_importances_
    indices = np.argsort(importancias)[::-1][:10]
    ...
    ax.grid(True, alpha=0.3, axis='x')
    plt.tight_layout()
    plt.savefig(f'importancia_xgb_binaria.png', dpi=300)
    print(f"  ✓ Salvo: importancia_xgb_binaria.png")
    plt.show()
...
def plotar_comparacao_metricas(resultados_multi, resultados_bin):
    ...
    plt.suptitle('Comparação de Métricas XGBoost: Multi-Classe vs Binária', fontsize=16, fontweight='bold')
    plt.tight_layout()
    plt.savefig('comparacao_metricas_xgb.png', dpi=300)
    print("\n✓ Salvo: comparacao_metricas_xgb.png")
    plt.show()
...
if __name__ == "__main__":
    print("="*70)
    print(" CLASSIFICAÇÃO COM XGBOOST - RESPOSTA AO NITROGÊNIO")
    print("="*70)
    ...
    # 3. Selecionar bandas (SEM adicionar novos índices, conforme pedido)
    ...
    print("\n" + "="*70)
    print(" CLASSIFICAÇÃO BINÁRIA (XGBOOST)")
    print("="*70)
