...
import sys
import optuna
import re  # <--- Necessário para extrair números do texto
...
SEED = 42
N_TRIALS = 20  # Recomendo aumentar para 50 se for usar o Dashboard
...
def extrair_numero_do_texto(valor):
    ...
    texto = str(valor)
    # Procura por dígitos (ex: 50, 100.5)
...
# ================= LÓGICA DO OPTUNA (COM STORAGE) =================

def otimizar_hiperparametros(X_train, y_train, study_name):
    """
    Otimiza hiperparâmetros salvando o progresso em um banco SQLite.
    """
...
def classificar_multiclasse(X, y, cols_nomes, titulo="Classificação Multi-Classe"):
    ...
    print(f"  🔍 Iniciando otimização com Optuna...")
    
    # Define um nome único para este estudo
    nome_estudo = "estudo_multiclasse"
...
def classificar_binaria(X, y_raw, cols_nomes, titulo="Classificação Binária"):
    # 1. Conversão Inteligente (Texto -> Número) usando Regex
    ...
    # 2. Criar labels binários
    # Se valor > 0 é "Com N", senão "Sem N"
    y_binary = np.where(y_numerico > 0, "Com N", "Sem N")
    
    # VERIFICAÇÃO DE SEGURANÇA
    classes_unicas = np.unique(y_binary)
    if len(classes_unicas) < 2:
        print(f"\n  ❌ ERRO CRÍTICO: Apenas uma classe encontrada: {classes_unicas}")
        print("     O modelo binário não pode ser treinado.")
        print("     Verifique se a coluna 'Tratamento' contém valores numéricos ou indicativos de dose (T1, T2...).")
...
if __name__ == "__main__":
    ...
    # Tratamento Multi-classe (Texto)
    y_tratamento = df['Tratamento'].astype(str)
    
    # Executa Multi-classe
    resultados_multi = classificar_multiclasse(X, y_tratamento, cols_selecionadas)
    
    # Executa Binária (Passando a coluna bruta de texto 'Tratamento' para ser processada dentro da função)
